...
export function getRolePermissions(role?: UserRole, _legacyEmail?: string): RolePermissions {
  const r = getRoleDisplay(role, _legacyEmail).role;
  switch (r) {
    case 'ADMIN':
      return {
...
export function getRoleDisplay(role?: UserRole, _legacyEmail?: string) {
  if (role === 'ADMIN') {
    return {
      role: 'ADMIN' as const,
...
export interface UserProfile {
  ...
  readonly sessionKind: 'firebase_session' | 'synthetic_demo';
  email: string;
  name: string;
  nickname?: string;
  legalName?: string;
...
export const TENANT_PRIVATE_PREVIEW = 'fam_private_preview';
export const TENANT_DEMO_SANDBOX = 'fam_demo_sandbox';

export interface FamilyVaultMember {
  email: string;
  role: UserRole;
  name: string;
  relation?: 'SELF' | 'FATHER' | 'MOTHER' | 'SPOUSE' | 'CHILD' | 'HUF' | 'ADVISOR' | 'OTHER';
  pan?: string;
  landingScreen?: 'dashboard' | 'holdings' | 'tax' | 'importer' | 'father-mode';
  avatarId?: string;
  masked?: boolean;
  portfolioId?: string;
  createdAt?: string;
...
export interface UserRegistryEntry {
  ...
  role: UserRole;
  name?: string;
  defaultLandingScreen?: 'dashboard' | 'holdings' | 'tax' | 'importer' | 'father-mode';
  createdAt?: string;
  lastLoginAt?: string;
}
